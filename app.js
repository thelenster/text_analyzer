...
function reportOnText(text){
	var tokens = tokenizeText(text);  //I don't understand when calling the function where(text)is coming from
	...
	textReport.find('.js-wordcount-result').text(numTotalWords);
	textReport.find('.js-uniqueword-count').text(numDistinctWords);
	textReport.find('.js-averageword-length').text(averageWordLength + ' characters');
